...
-- TODO
select `Station Area` from fireIncidents where  
(`Incident Date` like '04/04/%' or `Incident Date` like '09/27/%')
and `Station Area` > 20
...
-- TODO
select `Ignition Cause`,count(`Incident Number`) from fireIncidents group by `Ignition Cause`
...
select `Ignition Cause`,count(*) from fireIncidents group by `Ignition Cause` order by count(*) desc
...
select count(*) from fireCalls fc, fireIncidents fi where fc.Battalion = fi.Battalion limit 10
...
select count(*) from fireCalls fc, fireIncidents fi where fc.Battalion = fi.Battalion
